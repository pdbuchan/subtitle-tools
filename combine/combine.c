...
//  fprintf (stdout, "Sub: %i %02i:%02i:%02i,%03i --> %02i:%02i:%02i,%03i\n", i + 1, start[i].h, start[i].m, start[i].s, start[i].ms, end[i].h, end[i].m, end[i].s, end[i].ms);
  ...
  for (sub=1; sub<nsubs; sub++) {
    ...
    // If current sub's start time matches previous end time and text is identical then this a duplicate subtitle.
    if ((start[sub].totalms == end[sub - 1].totalms) && (strncmp (text[sub], text[sub - 1], MAXLEN) == 0)) {
      grouplist[sub] = group;
  ...
  do {
    ...
    // Find last matching subtitle endpt.
    endpt = sub;
    seek (grouplist, sub, &endpt, nsubs);
...
seek (int *grouplist, int first, int *try, int max) {
  ...
  // Don't go beyond end of grouplist.
  if ((*try) == max) {
    if ((*try) > first) (*try)--;
    return (EXIT_SUCCESS);
  }

  // If no longer within current group, return to current group by 
  // reducing sub number by one and return. Don't go below first sub in group.
  if (grouplist[first] != grouplist[*try]) {
    if ((*try) > first) (*try)--;
    return (EXIT_SUCCESS);
  }

  // Try next subtitle to see if it's also within current group.
  (*try)++;
  seek (grouplist, first, try, max);
