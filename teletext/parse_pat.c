...
parse_pat (STATE *state, PAT *pat, SECTION *section, FILE *fo) {
  ...
  if (table_id != 0x00 || ssi != 1 || section_length < 9 || section_length > 1021 || !bytes_available (0, section_length + 3, section[pid].length)) {
    return (EXIT_FAILURE);
  }
  ...
  tsid = (uint16_t) (((uint16_t) section[pid].buffer[offset] << 8) | section[pid].buffer[offset + 1]);
  offset += 2;
  ...
  fprintf (fo, "  Table ID: 0x%02x Section Length: %zu Transport Stream ID: 0x%04x Version: %u Current: %u Section: %u/%u\n", table_id, section_length, tsid, version, current, section_number, last_section);
