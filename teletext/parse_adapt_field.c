...
parse_adapt_field (STATE *state, size_t *index, uint8_t *tsdata, size_t tslen, FILE *fo) {
  ...
  if (pcr_flag) {
    ...
    pcr_ext = (uint16_t) ((((uint16_t) tsdata[*index + 4] & 1U) << 8) | (uint16_t) tsdata[*index + 5]);
  ...
  if (opcr_flag) {
    ...
    opcr_ext = (uint16_t) ((((uint16_t) tsdata[*index + 4] & 1U) << 8) | (uint16_t) tsdata[*index + 5]);
  ...
  if (adapt_field_ext_flag) {
    ...
    if (seamless_splice_flag) {
      ...
      // Decoding Time Stamp (DTS) Next Access Unit (33 bits). The value
      // is at most 33 bits, so it is safe to convert the completed unsigned
      // expression to int64_t after reassembly.
      {
        uint64_t dts_ticks;
        ...
        dts_next_au.totalms = (int64_t) ((dts_ticks + 45) / 90);
      }

      mstotime (&dts_next_au);
      fprintf (fo, "      Decoding Time Stamp (DTS) Next Access Unit (33 bits): %02i:%02i:%02i,%03i  totalms: %" PRId64 " ms\n", dts_next_au.h, dts_next_au.m, dts_next_au.s, dts_next_au.ms, dts_next_au.totalms);
