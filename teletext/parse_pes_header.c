...
read_timestamp (const uint8_t *p, uint8_t expected_prefix, int64_t *totalms) {
  ...
  if (((p[0] >> 4) & 0x0f) != expected_prefix || !(p[0] & 1) || !(p[2] & 1) || !(p[4] & 1)) {
    return (EXIT_FAILURE);
  }
...
int
parse_pes_header (STATE *state, size_t *offset, SEGMENT *segment, PES *pes, FILE *fo) {
  ...
  // Timestamp flags belong to this PES packet; never let values from an
  // earlier packet on the same PID leak into the current Teletext page.
  pes->have_pts = 0;
  pes->have_dts = 0;
  pes->data_alignment_indicator = 0;
  memset (&pes->pts, 0, sizeof (pes->pts));
  memset (&pes->dts, 0, sizeof (pes->dts));
  ...
  *offset += 2;
  ...
  pes->data_alignment_indicator = (flags1 >> 2) & 1;
  ...
  // DSM Trick Mode (1 byte). DVB Teletext does not normally need this field,
