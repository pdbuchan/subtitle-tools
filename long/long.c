...
#define BOM_BUFFER_SIZE 4  // Maximum Byte Order Mark (BOM) length
...
main (int argc, char **argv) {
  ...
  uint8_t bominput[BOM_BUFFER_SIZE] = {0};
  FILE *fi, *fo;
  ...
  bomread = fread (bominput, sizeof (bominput[0]), BOM_BUFFER_SIZE, fi);
  ...
  type = byteordermark (bominput, bomread, bom, nbom);
  if (type < 0) {
  ...
  if (fseek (fi, (type == 0) ? (long) bom[type].len : 0L, SEEK_SET) != 0) {
    fprintf (stderr, "ERROR: Unable to position input SubRip file %s after Byte Order Mark check.\n", filename);
    fclose (fi);
    exit (EXIT_FAILURE);
  }
  ...
  if (fseek (fi, (type == 0) ? (long) bom[type].len : 0L, SEEK_SET) != 0) {
    fprintf (stderr, "ERROR: Unable to reposition input SubRip file %s for reading.\n", filename);
    fclose (fi);
    exit (EXIT_FAILURE);
  }
