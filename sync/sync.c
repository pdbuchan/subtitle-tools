...
main (int argc, char **argv) {
  ...
  char *temp, *filename, **input, *timestamp;
  double ratio;
  BOM *bom;
...
//  fprintf (stdout, "Sub: %i %02i:%02i:%02i,%03i --> %02i:%02i:%02i,%03i\n", i + 1, start[i].h, start[i].m, start[i].s, start[i].ms, end[i].h, end[i].m, end[i].s, end[i].ms);
  ...
  // Ask for current start timestamp of first anchor point SubRip subtitle.
  ...
  // Ask for current start timestamp of last anchor point SubRip subtitle.
  fprintf (stdout, "Current start timestamp for last anchor point subtitle (hh:mm:ss,ms)? ");
  inputtext (timestamp);
  parsetimestamp (timestamp, &inputtime);
  oldlastms = inputtime.totalms;

  // Ask for new start timestamp of first anchor point SubRip subtitle.
  fprintf (stdout, "New start timestamp for first anchor point subtitle (hh:mm:ss,ms)? ");
  inputtext (timestamp);
  parsetimestamp (timestamp, &inputtime);
  newfirstms = inputtime.totalms;

  // Ask for new start timestamp of last anchor point SubRip subtitle.
  fprintf (stdout, "New start timestamp for last anchor point subtitle (hh:mm:ss,ms)? ");
  inputtext (timestamp);
  parsetimestamp (timestamp, &inputtime);
  newlastms = inputtime.totalms;

  // Synchronize all timestamps to new anchor points.
  ratio = (((double) newlastms - (double) newfirstms) / ((double) oldlastms - (double) oldfirstms));
  for (i=0; i<nsubs; i++) {

    // Synch start timestamp total milliseconds.
    start[i].totalms = (int64_t) (((double) newfirstms) + ((((double) start[i].totalms) - ((double) oldfirstms)) * ratio));

    // Update h, m, s, ms in TIME struct.
    mstotime (&start[i]);

    // Synch end timestamp total milliseconds.
  ...
  for (i=0; i<nsubs; i++) {
    ...
    // Write timestamp.
    fprintf (fo, "%02i:%02i:%02i,%03i --> %02i:%02i:%02i,%03i\n", start[i].h, start[i].m, start[i].s, start[i].ms, end[i].h, end[i].m, end[i].s, end[i].ms);
    line++;
