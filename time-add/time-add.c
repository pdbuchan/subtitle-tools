...
// time-add.c - Given two timestamps from standard input, calculate the sum and output as a timestamp.

// gcc -std=c11 -Wall -Wextra -Wpedantic time-add.c -o time-add

// Run without command line arguments.
// Input: starting timestamp, additional time as a timestamp
...
main (void) {
  ...
  // Ask for additional timestamp.
  fprintf (stdout, "Additional time as a timestamp (hh:mm:ss.xxx or hh:mm:ss,xxx)? ");
  inputtext (timestamp);
  parsetimestamp (timestamp, &add);

  // Calculate sum and express as a timestamp.
  if (add.totalms > (INT64_MAX - start.totalms)) {
    fprintf (stderr, "ERROR: Timestamp sum is too large.\n");
    return (EXIT_FAILURE);
  }
  sum.totalms = start.totalms + add.totalms;
  mstotime (&sum);

  // Show sum on standard output.
  fprintf (stdout, "Sum is (hh:mm:ss.ms): %02i:%02i:%02i.%03i\n\n", sum.h, sum.m, sum.s, sum.ms);
...
parsetimestamp (const char *timestamp, TIME *time) {
  ...
  if ((timestamp[2] != ':') || (timestamp[5] != ':') || ((timestamp[8] != '.') && (timestamp[8] != ','))) {
