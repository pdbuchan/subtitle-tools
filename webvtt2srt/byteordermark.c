...
//
// UTF-7 has four complete BOM forms. Testing only the common three-byte
// prefix "+/v" would incorrectly identify ordinary text beginning with, for
// example, "+/vX" as UTF-7.
...
// Table of recognized BOMs. The order is not used to resolve prefix matches;
// byteordermark() deliberately selects the longest complete match.
const BOM bom[] = {
...
int
byteordermark (const uint8_t *text, size_t nbytes, const BOM *list, size_t nlist) {
